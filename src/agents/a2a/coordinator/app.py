...
class CompanyResearchRequest(BaseModel):
    ...
    max_iterations: int = Field(
        default=3,
        ge=1,
        le=5,  # Configuration.max_reflection_steps allows at most 5
        description="Maximum research → extraction → reflection cycles",
    )
...
async def research_company(request: CompanyResearchRequest):
    ...
    try:
        ...
        # reflection_node stops the loop once reflection_count reaches max_reflection_steps
        config = Configuration(max_reflection_steps=request.max_iterations)
        ...
        for iteration in range(request.max_iterations):
            ...
            # reflection_node increments reflection_count and sets is_complete / follow_up_queries
            reflection_result = await reflection_node(state, config)

            state.update(reflection_result)
            reflection_messages = reflection_result.get("messages", [])
            if reflection_messages:
                state["reflection_summary"] = reflection_messages[-1].get("content", "")

            logger.info(f"Reflection completed: is_complete={state['is_complete']}")
