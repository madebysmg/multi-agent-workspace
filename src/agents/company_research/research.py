...
def get_google_search_wrapper() -> Optional[Any]:
    """
    Create a Google Programmable Search (Custom Search JSON API) wrapper.

    Used by the "google_adk" and "hybrid" providers. Needs the
    langchain-google-community package plus GOOGLE_API_KEY and GOOGLE_CSE_ID.

    Returns:
        A GoogleSearchAPIWrapper instance, or None if it cannot be created
    """
    try:
        from langchain_google_community import GoogleSearchAPIWrapper
    except ImportError:
        print("Warning: langchain-google-community not installed. "
    ...
    try:
        ...
        # Do not print the exception itself: validation errors can echo the API key.
        print(f"Warning: Google search is not configured ({type(e).__name__}). "
...
async def research_node(state: ResearchState, config: Configuration) -> Dict[str, Any]:
    ...
    if config.search_provider == "tavily":
        ...
        # Google Programmable Search (Custom Search JSON API) via langchain-google-community.
        # Despite the provider name, this does not use Google ADK or Gemini.
        google_search = get_google_search_wrapper()

        if google_search is not None:
            for query in queries[:config.max_search_queries]:
                try:
                    # The wrapper is synchronous and returns a string, so run it in a thread
                    result_text = await asyncio.to_thread(google_search.run, query)
                    all_results.append({
                    ...
                    # Only the error type: googleapiclient errors include the request URL with the API key
                    print(f"Google search error for query '{query}': {type(e).__name__}")
                    continue
        ...
        # First half of the queries with Tavily, second half with Google Programmable Search
        ...
        # Google Programmable Search for the second half
        google_search = get_google_search_wrapper()
        ...
        if google_search is not None:
            for query in queries[mid_point:config.max_search_queries]:
                try:
                    ...
                    print(f"Google search (hybrid) error for '{query}': {type(e).__name__}")
                    continue
        else:
            print("Google search unavailable for hybrid mode, using Tavily for all queries")
        ...
        try:
            ...
            print("Warning: DuckDuckGo search package not installed. Install with: pip install -U ddgs")
            print("Cannot proceed without a search provider.")
            return {
                "research_queries": queries,
                "search_results": [],
                "research_notes": "Error: No search provider available. Please install ddgs or configure another provider.",
