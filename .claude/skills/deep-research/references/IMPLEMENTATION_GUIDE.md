...
This guide provides code for implementing the company research agent. The existing implementation at `src/agents/company_research/` (plus `src/common/llm.py` and `src/common/utils.py`) already contains these files - use this as reference when customizing or understanding the system.

> **Current code limits** (checked against `src/`): `src/common/llm.py` always creates `ChatAnthropic`, so the DeepSeek/Qwen/Gemini configurations below need a change to `get_llm()` first. The `google_adk` provider is Google Programmable Search via `langchain-google-community` (needs `GOOGLE_API_KEY` + `GOOGLE_CSE_ID`), not Gemini's free `google_search`, and `use_gemini_for_google_search` is unused. Prices are 2025-10 estimates.
...
config = Configuration(
    ...
    search_provider="google_adk"  # Google Programmable Search
)
...
## File 3: llm.py - Rate Limiting (`src/common/llm.py`)
...
## File 4: utils.py - Shared Utilities (`src/common/utils.py`)
...
See `src/common/utils.py` for complete implementations (`llm.py` is also in `src/common/`).
...
- Tavily (best quality)
- google_adk (Google Programmable Search, falls back to Tavily)
- Hybrid (half Tavily, half Google)
- SerpAPI
- Bing
- DuckDuckGo (free)
- Brave
...
def build_research_graph(config: Configuration):
    ...
    # Add nodes. The node functions are async, so bind config with async
    # wrappers; a sync lambda would return an un-awaited coroutine.
...
4. **Error handling**: failed queries are logged and skipped; `google_adk`/`hybrid` fall back to Tavily
...
## Performance (design estimates, not measured)

- Processing: 45-90 seconds per company
- Cost (Tavily): $0.015-0.045 per company
- Cost (Hybrid): about half the Tavily cost, plus Google search charges
- Target: 85%+ schema completeness
