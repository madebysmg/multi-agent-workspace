...
1. **Set all required API keys**
   ...
   export GOOGLE_API_KEY="your_key"  # For google_adk / hybrid
   export GOOGLE_CSE_ID="your_id"    # Programmable Search Engine ID
   ```
...
### google_adk (Google Programmable Search)

**Issue:** Log shows "Google search is not configured" or "langchain-google-community not installed", and results come from Tavily
**Solution:**
```bash
...
```
The provider does not use Gemini; `use_gemini_for_google_search` has no effect.
