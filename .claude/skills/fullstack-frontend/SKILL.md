...
description: Next.js 14 + shadcn/ui frontend builder. Use this skill when building web UI for backend APIs. Supports TypeScript, Tailwind CSS, App Router, API integration, and Vercel deployment.
license: MIT
---

# Fullstack Frontend Skill

Build Next.js 14 frontend applications with shadcn/ui, TypeScript, and Tailwind CSS.

> **Template status**: the bundled `assets/nextjs-template/` is incomplete. It does not include `lib/api.ts`, `lib/types.ts`, `lib/utils.ts`, a PostCSS config, the `/history` page or the custom components (`research-form`, `result-viewer`, `history-table`, `stats-card`) described below, so it does not build as-is. Add those files (or ask Claude to generate them from this guide) before running `npm run build`.
...
- **Deployment notes** - Guidance for Vercel, AWS or other platforms (the bundled template is incomplete, see the status note above)
