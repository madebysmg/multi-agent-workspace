...
Analyze and transplant multi-agent workspace architecture patterns to other projects, enabling reuse of its components and design patterns.
...
**Implementation**: Generate with `python scripts/scaffold_agent.py --type langgraph ...` (creates `state.py`), or copy the workspace version with `python scripts/extract_component.py --component state ...`.
...
**Implementation**: Generate with `python scripts/scaffold_agent.py --type langgraph ...` (creates `configuration.py`), or copy the workspace version with `python scripts/extract_component.py --component configuration ...`.
...
**Implementation**: Copy `src/common/utils.py` with `python scripts/extract_component.py --component utils ...`.
...
### Templates (generated, not stored)
No template files are stored under `assets/` (see `assets/README.md`). They are produced by the scripts:
- **`scripts/scaffold_agent.py --type langgraph`**: LangGraph agent skeleton (`__init__.py`, `state.py`, `configuration.py`, `prompts.py`, `graph.py`)
- **`scripts/scaffold_agent.py --type a2a`**: A2A agent with FastAPI
- **`scripts/extract_component.py --component utils|llm|prompts|state|configuration|graph`**: copies an existing component from the workspace
...
Check if a component already exists before writing new code. Use `extract_component.py` to reuse existing implementations.
...
**Time**: 1-2 hours
**Result**: An agent skeleton with centralized prompts, state and configuration, ready to customize.
