...
> A2A 기반 분산 멀티 에이전트 시스템 - 기업 리서치 자동화 프로토타입

**최종 업데이트**: 2025-10-22
**현재 버전**: v2.0.0 (LangGraph 모놀리식)
**다음 버전**: v3.0.0 (A2A 분산 시스템) ⭐ 설계 완료
**프로젝트**: Company Research Automation (prototype, 자동화된 테스트 없음)

> **수치 안내**: 이 문서의 성능·비용 수치(처리 시간, 480배, 월 $5,222 → $464, 91% 절감 등)는 설계 단계의 추정치입니다. 저장소에는 이를 뒷받침하는 벤치마크나 운영 측정 결과가 포함되어 있지 않습니다.
...
2. **성능 (추정)**: 12시간 → **90초** (480배 빠름)
3. **비용 (추정)**: 월 $5,222 → **$464** (91% 절감)
4. **확장성**: 독립적 에이전트 스케일링
...
- 91% 비용 절감 추정 ($5,222 → $464)
- 병렬 처리 가능
- 자동 복구

**상세**: [COST_OPTIMIZATION.md](docs/COST_OPTIMIZATION.md)
...
- [x] Extraction Phase (`extraction.py`)
  - JSON 데이터 추출 (`JsonOutputParser`)
  - 파싱 실패 시 스키마 필드를 `null`로 채운 기본값 반환
...
- [x] Claude Code 스킬 (8개, `.claude/skills/`)
...
- [x] Terraform 예시 코드 (설계 문서 내 스니펫, 별도 `terraform/` 디렉터리는 없음)

### v3.0 구현 예정 항목

#### Phase 1: Hybrid Wrapper (2주)

- [x] Research Agent A2A 래퍼 (FastAPI, `src/agents/a2a/research_agent/`)
- [x] Extraction Agent A2A 래퍼 (FastAPI, `src/agents/a2a/extraction_agent/`)
- [ ] Reflection Agent → Lambda 전환 (현재는 Coordinator 안에서 로컬 실행)
- [x] Coordinator FastAPI 서버 (`src/agents/a2a/coordinator/`)
- [x] Docker Compose 설정 (`docker-compose.a2a.yml`)
- [ ] 로컬 테스트 (10개 회사)
...
from src.agents.company_research.research import research_node  # 기존 코드!
...
| 문서 | 설명 |
|------|------|
| **[A2A_ARCHITECTURE.md](docs/A2A_ARCHITECTURE.md)** | 전체 시스템 아키텍처 (42KB) |
| **[INFRASTRUCTURE_DESIGN.md](docs/INFRASTRUCTURE_DESIGN.md)** | AWS 인프라 + Terraform 스니펫 (29KB) |
| **[COST_OPTIMIZATION.md](docs/COST_OPTIMIZATION.md)** | 비용 최적화 전략 (91% 절감 추정) |
...
| 문서 | 설명 |
|------|------|
| [GOOGLE_ADK_ANALYSIS.md](docs/GOOGLE_ADK_ANALYSIS.md) | Google ADK 분석 (비용 절감) |
| [COMPARISON_ANALYSIS.md](docs/COMPARISON_ANALYSIS.md) | company-researcher 비교 |
| [IMPROVEMENTS_APPLIED.md](docs/IMPROVEMENTS_APPLIED.md) | v2.0 개선사항 |
...
| [GETTING_STARTED.md](docs/GETTING_STARTED.md) | 빠른 시작 가이드 |
| [CREATE_AGENT.md](docs/CREATE_AGENT.md) | 새 에이전트 만들기 |
| [README_DEEP_RESEARCH.md](docs/README_DEEP_RESEARCH.md) | 상세 사용 가이드 |
| [src/agents/a2a/README.md](src/agents/a2a/README.md) | A2A 서비스 실행 방법 |
...
| 항목 | 수량 |
|------|------|
| Python 파일 (`src/`, `examples/`) | 24개 |
| 코드 라인 (`src/`, `examples/`) | ~2,900 |
| Phase 구현 | 3개 |
| 예제 | 6개 |
| 문서 (`docs/`) | 11개 |
...
- ✅ Research → Extract → Reflect 루프 구현
- ✅ Rate limiting, 토큰 제한, 중복 제거
- ✅ 코드 품질 향상 (프롬프트 중앙화, utils 분리)
- ✅ 45-90초/회사 처리 속도 (문서상 수치)
...
**작성**: 2025-10-22
**버전**: v2.0.0 (prototype), v3.0.0 (design)
